...
fun AdvancedInferenceSection(
    ...
    onNKeepChange: (Int) -> Unit,
    onDefragTholdChange: (Float) -> Unit,
    onContextShiftChange: (Boolean) -> Unit,
    onFullSwaCacheChange: (Boolean) -> Unit,
    onSaveLocalKvChange: (Boolean) -> Unit
) {
    ...
    SettingsSection(title = stringResource(R.string.settings_inference)) {
        Column(
            ...
            HorizontalDivider(color = MaterialTheme.colorScheme.surfaceVariant, modifier = androidx.compose.ui.Modifier.padding(vertical = 8.dp))

            // ─── Memory & Context Advanced Settings ───
            ...
            Row(
                ...
                Column(modifier = androidx.compose.ui.Modifier.weight(1f)) {
                    Text("Use Memory Mapping (mmap)", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
                    Text("Map model file directly — faster load, less RAM copy", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Switch(checked = settings.useMmap, onCheckedChange = onUseMmapChange)
            }
            ...
            Row(
                ...
                Column(modifier = androidx.compose.ui.Modifier.weight(1f)) {
                    Text("Lock Model in RAM (mlock)", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
                    Text("Prevent OS from swapping model pages — needs spare RAM", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Switch(checked = settings.useMlock, onCheckedChange = onUseMlockChange)
            }

            // n_keep slider (0 – 1024)
            RealTimeSlider(
                title = "Prefix Keep (n_keep): ${settings.nKeep} tokens",
                value = settings.nKeep.toFloat(),
                valueRange = 0f..1024f,
                steps = 127,
                onValueChangeFinished = { onNKeepChange(it.toInt()) },
                formatValue = { it.toInt().toString() }
            )

            // defrag_thold slider (0% – 100%)
            RealTimeSlider(
                title = "KV Defrag Threshold: ${(settings.defragThold * 100).toInt()}%",
                value = settings.defragThold,
                valueRange = 0f..1f,
                onValueChangeFinished = { onDefragTholdChange(it) },
            ...
            Row(
                ...
                Column(modifier = androidx.compose.ui.Modifier.weight(1f)) {
                    Text("Context Shift", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
                    Text("Slide context window instead of failing on overflow", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Switch(checked = settings.contextShift, onCheckedChange = onContextShiftChange)
            }
            ...
            Row(
                ...
                Column(modifier = androidx.compose.ui.Modifier.weight(1f)) {
                    Text("Full SWA Cache", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
                    Text("Full sliding window attention KV cache — higher quality, more RAM", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Switch(checked = settings.fullSwaCache, onCheckedChange = onFullSwaCacheChange)
            }
            ...
            Row(
                ...
                Column(modifier = androidx.compose.ui.Modifier.weight(1f)) {
                    Text("Save Local KV Cache", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
                    Text("Persist KV cache to disk for faster session resumption", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Switch(checked = settings.saveLocalKv, onCheckedChange = onSaveLocalKvChange)
            }
        }
    }
}
