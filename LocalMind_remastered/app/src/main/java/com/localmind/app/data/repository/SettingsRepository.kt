...
class SettingsRepository @Inject constructor(
    ...
    companion object {
        ...
        const val DEFAULT_N_KEEP = 1024
        const val DEFAULT_DEFRAG_THOLD = 0.5f // 50%
    }
    ...
    private object PreferencesKeys {
        ...
        // Advanced memory / context settings
        val USE_MLOCK = booleanPreferencesKey("use_mlock")
        val USE_MMAP = booleanPreferencesKey("use_mmap")
        val N_KEEP = intPreferencesKey("n_keep")
        val DEFRAG_THOLD = floatPreferencesKey("defrag_thold")
        val CONTEXT_SHIFT = booleanPreferencesKey("context_shift")
        val FULL_SWA_CACHE = booleanPreferencesKey("full_swa_cache")
        val SAVE_LOCAL_KV = booleanPreferencesKey("save_local_kv")
    }
    ...
    val useMlock: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.USE_MLOCK] ?: false }
    val useMmapExplicit: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.USE_MMAP] ?: true }
    val nKeep: Flow<Int> = dataStore.data.map { it[PreferencesKeys.N_KEEP] ?: DEFAULT_N_KEEP }
    val defragThold: Flow<Float> = dataStore.data.map { it[PreferencesKeys.DEFRAG_THOLD] ?: DEFAULT_DEFRAG_THOLD }
    val contextShift: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.CONTEXT_SHIFT] ?: true }
    val fullSwaCache: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.FULL_SWA_CACHE] ?: false }
    val saveLocalKv: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.SAVE_LOCAL_KV] ?: false }
    ...
    suspend fun updateNKeep(value: Int) {
        dataStore.edit { it[PreferencesKeys.N_KEEP] = value }
    }

    suspend fun updateDefragThold(value: Float) {
        dataStore.edit { it[PreferencesKeys.DEFRAG_THOLD] = value }
    }

    suspend fun updateContextShift(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.CONTEXT_SHIFT] = enabled }
    }

    suspend fun updateFullSwaCache(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.FULL_SWA_CACHE] = enabled }
    }

    suspend fun updateSaveLocalKv(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.SAVE_LOCAL_KV] = enabled }
    }
