...
class LLMEngine @Inject constructor(
    ...
    suspend fun loadModel(
        ...
        try {
            ...
            // Se respetan EXACTAMENTE los valores del usuario. Antes se forzaba batch >= 2048,
            // ubatch >= 512, Flash Attention "Auto" -> "On" y cache F16 -> Q8_0, lo que hacia
            // que los ajustes de la pantalla fueran decorativos y podia fallar o ir lento en
            // CPUs modestas. Solo se exige >= 1 para no pasar ceros/negativos al motor.
            val batchSize = settingsRepository.batchSize.first().coerceAtLeast(1)
            // n_ubatch nunca puede superar n_batch en llama.cpp
            val physicalBatchSize = settingsRepository.physicalBatchSize.first()
                .coerceAtLeast(1)
                .coerceAtMost(batchSize)
            // "Auto" se pasa tal cual: lo resuelve llama.cpp segun el backend disponible.
            val flashAttention = settingsRepository.flashAttention.first()
            val keyCacheType = settingsRepository.keyCacheType.first()
            val valueCacheType = settingsRepository.valueCacheType.first()
            ...
            // useMmapExplicit overrides the legacy memoryMapping string setting when set
            val useMmapOverride = settingsRepository.useMmapExplicit.first()
            val nKeep = settingsRepository.nKeep.first().coerceIn(0, 131072)
            val defragThold = settingsRepository.defragThold.first().coerceIn(0f, 1f)
            val contextShift = settingsRepository.contextShift.first()
            val fullSwaCache = settingsRepository.fullSwaCache.first()
            val saveLocalKv = settingsRepository.saveLocalKv.first()
            ...
            try {
                ...
                Log.i(
                    TAG,
                    "Loading model: $nativePath [mlock=$allowMlock, mmap=$useMmapOverride, ctx=$tunedContextSize, threads=$tunedThreadCount, gpu=$requestedGpuLayers, n_keep=$nKeep, defrag=$defragThold, shift=$contextShift, swa=$fullSwaCache, save_kv=$saveLocalKv]"
                )
