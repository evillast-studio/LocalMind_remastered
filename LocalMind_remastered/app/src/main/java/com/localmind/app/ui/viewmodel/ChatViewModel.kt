...
class ChatViewModel @Inject constructor(
    ...
    fun createNewConversation() {
        viewModelScope.launch {
            ...
            // If the active persona has a firstMessage, inject it as the opening assistant turn
            val firstMsg = _state.value.selectedPersona?.firstMessage?.trim()
            if (!firstMsg.isNullOrBlank()) {
                val opening = com.localmind.app.domain.model.Message(
                    conversationId = conversation.id,
                    content = firstMsg,
                    role = com.localmind.app.domain.model.MessageRole.ASSISTANT,
                    timestamp = System.currentTimeMillis()
                )
                chatRepository.addMessageSafely(opening)
            }
    ...
    private suspend fun buildPromptWithHistory(
        ...
        // Build enriched system prompt from character card fields
        val baseSystemPrompt = conversation?.systemPrompt?.trim()?.takeIf { it.isNotEmpty() }
            ?: persona?.systemPrompt?.trim()?.takeIf { it.isNotEmpty() } ?: DEFAULT_SYSTEM_PROMPT
        val effectiveSystemPrompt = buildString {
            ...
            if (persona != null) {
                ...
                if (persona.exampleMessages.isNotBlank()) {
                    append("\n\n[Example Dialogue]:\n${persona.exampleMessages}")
                }
            }
        }
