...
data class SettingsState(
    ...
    val nKeep: Int = SettingsRepository.DEFAULT_N_KEEP,
    val defragThold: Float = SettingsRepository.DEFAULT_DEFRAG_THOLD,
    val contextShift: Boolean = true,
    val fullSwaCache: Boolean = false,
    val saveLocalKv: Boolean = false
)
...
class SettingsViewModel @Inject constructor(
    ...
    val settings: StateFlow<SettingsState> = combine(
        ...
        repository.useMlock,
        repository.useMmapExplicit,
        repository.nKeep,
        repository.defragThold,
        repository.contextShift,
        repository.fullSwaCache,
        repository.saveLocalKv
    ) { args ->
        SettingsState(
            ...
            useMlock = args[32] as Boolean,
            useMmap = args[33] as Boolean,
            nKeep = args[34] as Int,
            defragThold = args[35] as Float,
            contextShift = args[36] as Boolean,
            fullSwaCache = args[37] as Boolean,
            saveLocalKv = args[38] as Boolean
        )
    ...
    fun updateNKeep(value: Int) = viewModelScope.launch {
        repository.updateNKeep(value)
    }

    fun updateDefragThold(value: Float) = viewModelScope.launch {
        repository.updateDefragThold(value)
    }

    fun updateContextShift(enabled: Boolean) = viewModelScope.launch {
        repository.updateContextShift(enabled)
    }

    fun updateFullSwaCache(enabled: Boolean) = viewModelScope.launch {
        repository.updateFullSwaCache(enabled)
    }

    fun updateSaveLocalKv(enabled: Boolean) = viewModelScope.launch {
        repository.updateSaveLocalKv(enabled)
    }
